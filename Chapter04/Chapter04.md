...
![X等待登陆的画面示意图](image/centos7_x_login_01.jpg)图4.1.1、X等待登陆的画面示意图

接下来让我们来了解一下这个登陆画面的相关功能吧！首先，在箭头 1 的地方，如果你动鼠标过去点一下，就会出现如下的窗口， 主要在告诉你日期、日历与时间而已～如下图所示，鸟哥撷取这张图的时间就是在 2015/05/21 早上喔！

![X等待登陆的画面示意图-日历、时间显示](image/centos7_x_login_02.jpg)图4.1.2、X等待登陆的画面示意图-日历、时间显示
...
![X等待登陆的画面示意图-无障碍登陆协助](image/centos7_x_login_03.jpg)图4.1.3、X等待登陆的画面示意图-无障碍登陆协助
...
![X等待登陆的画面示意图-无须登陆的关机与重新开机](image/centos7_x_login_05.jpg)图4.1.4、X等待登陆的画面示意图-无须登陆的关机与重新开机
...
![X等待登陆的画面示意图-一般帐号登陆系统的密码字段](image/centos7_x_login_08.jpg)图4.1.5、X等待登陆的画面示意图-一般帐号登陆系统的密码字段
...
![每个用户第一次登陆系统的环境设置](image/centos7_gnome_01.jpg)图4.1.6、每个用户第一次登陆系统的环境设置

再来则是选择输入法，除非你有特殊需求，否则不需要修改设置值。若是需要有其他不同的输入法，请看下图左侧箭头指的“+”符号， 按下它就可以开始选择其他的输入法了。一切顺利的话，请点选“下一步”。

![每个用户第一次登陆系统的环境设置](image/centos7_gnome_02.jpg)图4.1.7、每个用户第一次登陆系统的环境设置
...
![窗口接口的环境介绍](image/centos7_gnome_05.jpg)图4.1.9、窗口接口的环境介绍
...
![应用程序集当中，需要注意有阶层的显示喔！](image/centos7_gnome_06.jpg)图4.1.10、应用程序集当中，需要注意有阶层的显示喔！
...
![应用程序的总览画面示意图](image/centos7_gnome_07.jpg)图4.1.11、应用程序的总览画面示意图！
...
![文件资源管理器操作示意图](image/centos7_gnome_08.jpg)图4.1.12、文件资源管理器操作示意图
...
![文件资源管理器操作示意图](image/centos7_gnome_09.jpg)图4.1.13、文件资源管理器操作示意图
...
![文件资源管理器操作示意图](image/centos7_gnome_10.jpg)图4.1.14、文件资源管理器操作示意图
...
![文件资源管理器操作示意图](image/centos7_gnome_11.jpg)图4.1.15、文件资源管理器操作示意图
...
![地区与语言设置项目](image/centos7_gnome_12.jpg)图4.1.16、地区与语言设置项目
...
![输入法切换之快捷键设置](image/centos7_gnome_13.jpg)图4.1.17、输入法切换之快捷键设置
...
![离开窗口接口或 Linux 的方式](image/centos7_gnome_14.jpg)图4.1.18、离开窗口接口或 Linux 的方式：有登出、锁定与关机

不论是登出还是关闭电源（关机），都会有一个警告窗口来告知你 60 秒内没有任何动作的话，就会被登出了！如下图所示。当然， 你也可以按下确定来进行动作。登出后，系统画面又会回到原本的等待登陆的画面中了！

![离开窗口接口或 Linux 的方式](image/centos7_gnome_15.jpg)图4.1.19、离开窗口接口或 Linux 的方式：登出提醒
...
基本上，在默认情况下，这几个指令都会完成一样的工作（调用 `systemctl` 这个重要的管理命令）。你只要记得其中一个就好了，重点是你自己习惯即可。
