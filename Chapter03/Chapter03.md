...
![启动虚拟机管理员示意图](image/virtual_01.jpg)图3.2.1、启动虚拟机管理员示意图

因为我们是想要创建新的虚拟机，因此你要像上图那样，点选“文件”然后点选“ New Virtual Machine ”，接下来就能够看到如下图的模样来创建新机器！

![选择安装方式](image/virtual_02.jpg) ![选择安装光盘镜像](image/virtual_03.jpg)图3.2.2、选择使用光盘来安装，并实际选择 CentOS 镜像文件所在
...
![设置内存与 CPU 数](image/virtual_04.jpg)![选择磁盘容量](image/virtual_05.jpg)图3.2.3、设置内存容量、CPU数量、磁盘容量等重要机器设置
...
![光盘开机后安装画面之选择](image/centos7_01.jpg)图3.2.8、光盘开机后安装画面之选择
...
![统一按钮展示的安装画面](image/centos7_05.jpg)图3.2.13、时区挑选的项目示意图
...
![时区挑选的项目](image/centos7_06.jpg)图3.2.14、键盘配置项目
...
![新增其他语系的键盘配置](image/centos7_08.jpg)图3.2.15、新增其他语系的键盘配置
...
![准备被安装的软件放置在哪里呢](image/centos7_09.jpg)图3.2.16、挑选准备要被安装的软件所在的媒体
...
![选择要安装 Linux 的硬盘，并选择手动分区模式](image/centos7_11.jpg)图3.2.18、选择要安装 Linux 的硬盘，并选择手动分区模式
...
![删除分区时出现的警告窗口示意图](image/centos7_13.jpg)图3.2.20、删除分区时出现的警告窗口示意图
...
![单一分区分区完成详细项目示意图](image/centos7_15.jpg)
...
![创建 /boot 分区的示意图](image/centos7_16.jpg)图3.2.23、创建 /boot 分区的示意图
...
![创建根目录 / 的分区](image/centos7_17.jpg)图3.2.24、创建根目录 / 的分区
...
![将设备类型改为 LVM 的管理机制](image/centos7_18.jpg)图3.2.25、将设备类型改为 LVM 的管理机制
...
![修改与设置 LVM 设备的容量](image/centos7_19.jpg)图3.2.26、修改与设置 LVM 设备的容量
...
![KDUMP 的挑选示意图](image/centos7_26.jpg)图3.2.33、KDUMP 的挑选示意图
...
![网络设置示意图](image/centos7_27.jpg)图3.2.34、网络设置示意图
...
![设置开机自动启动网络](https://wizardforcel.gitbooks.io/vbird-linux-basic-4e/content/img/centos7_28.jpg)图3.2.35、设置开机自动启动网络

现在 CentOS 7 开机后，默认是没有启动网络的，因此你得要在上图中选择 2 号箭头的“当这个网络可用时自动连线”的项目才行！

![手动设置 IP 的示意图](image/centos7_29.jpg)图3.2.36、手动设置 IP 的示意图

如上图所示，选择 IPv4 的项目，然后调整 2 号箭头成为手动，接下来按下 3 号箭头加入项目后，才能够在 4 号箭头输入所需要的 IP 位址与网络遮罩～ 写完之后其他的项目不要更动，就按下 5 号箭头的储存吧！然后回到如同下图的画面：

![修改主机名称](image/centos7_30.jpg)图3.2.37、修改主机名称
...
![设置完毕并准备开始安装的示意图](image/centos7_31.jpg)图3.2.38、设置完毕并准备开始安装的示意图

现在的安装画面作的还挺简单的，省略了一堆步骤！上述画面按下开始安装后，这时你就可以一边让系统安装， 同时去设置其他项目，可以节省时间啦！如下图所示，还有两件重要的事件要处理，一个是 root 密码，一个是一般身份用户的创建！

![进行安装程序中，还可以持续其他任务的过程](image/centos7_32.jpg)图3.2.39、进行安装程序中，还可以持续其他任务的过程

将上图中，按下 ROOT 密码，可以得到下面的图示来修改系统管理员的密码喔！

![设置系统管理员 root 的密码](image/centos7_33.jpg)图3.2.40、设置系统管理员 root 的密码
...
![创建一个一般帐号](image/centos7_34.jpg)图3.2.41、创建一个一般帐号
...
![安装完毕的示意图](image/centos7_35.jpg)图3.2.42、安装完毕的示意图
...
![第一次使用 CentOS 7 图形接口的授权同意过程](image/centos7_36.jpg)图3.2.43、第一次使用 CentOS 7 图形接口的授权同意过程
...
![等待使用者登陆示意图](image/centos7_38.jpg)图3.2.45、等待使用者登陆示意图![鸟哥的图示](https://wizardforcel.gitbooks.io/vbird-linux-basic-4e/content/20.htmlimg/vbird_face.gif "鸟哥的图示")
...
![memory test 的图示](image/centos7_39.jpg)图3.2.46、memory test 的图示
...
```
[anaconda root@localhost /]# parted /dev/vda mklabel msdos
